...
import 'package:forge2d_eg/components/body_component_with_user_data.dart';

const brickScale = 0.5;
const brickCount = 5;

enum BrickType {
  explosive(density: 1.0, friction: 0.5),
  glass(density: 0.5, friction: 0.2),
  metal(density: 1.0, friction: 0.4),
  stone(density: 2.0, friction: 1.0),
  ...
  static BrickType get randomType =>
      values[math.Random().nextInt(values.length)];
}
...
enum BrickSize {
  ...
  static BrickSize get randomSize =>
      values[math.Random().nextInt(values.length)];
}

enum BrickDamage { none, some, lots }

Map<BrickDamage, String> brickFileNames(
  BrickType brickType,
  BrickSize brickSize,
) {
  return switch ((brickType, brickSize)) {
...
class Brick extends BodyComponentWithUserData {
         ...
         fixtureDefs: [
           FixtureDef(
               PolygonShape()..setAsBoxXY(
                 brickSize.size.width / 20 * brickScale,
                 brickSize.size.height / 20 * brickScale,
               ),
             )
             ..restitution = 0.4
             ..density = brickType.density
             ..friction = brickType.friction,
         ],
       );
  ...
  Future<void> onLoad() {
    _spriteComponent = SpriteComponent(
      ...
      size: brickSize.size.toVector2() / 10 * brickScale,
